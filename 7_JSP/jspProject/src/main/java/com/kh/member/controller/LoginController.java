...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser == null) {
			...
			// Servlet에서는 session영역에 접근하고자 하면 우선 세견객ㄱ체를 얻어야 한다.
			HttpSession session  = request.getSession();
			session.setAttribute("loginUser", loginUser);
			
			// 1. 포워딩 방식 -> 해당방식은 url이 변경되지 않는다.
			/*
				-> 우리는 localhost:8888/jsp/라는 기존의 메인 url이 있다.
			 	해당 방식으로응답시 url은 http://localhost:8888/jsp/login.me가 나타난다.
			 	실제 화면은 localhost:8888/jspProject/의 응답화면이 나타난다.
