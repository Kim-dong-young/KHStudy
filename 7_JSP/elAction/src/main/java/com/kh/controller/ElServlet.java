...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *    브라우저에서 요청을 할 때, 톰캣에서 세션을 생성
		 *    세션을 생성하고, 난수로 생성된 세션 id를 브라우저에 보내줌
		 *    브라우저는 쿠키로 저장함 - 서로 구분하기 위한 용도
		 *    쿠키-세션으로 로그인 구현을 많이한다.
		 *   
		 * 3. HttpServletRequest(request scope)
		 *    요청때마다 매번 생성되는 객체
		 *    이 영역에 데이터를 담으면 해당 request객체를 포워딩받는 응답 jsp에서만 사용 가능
		 *    
		 * 4. PageContext(page scope)
		 * 	  jsp마다 존재하는 객체
		 *    공유범위가 가장 작음(해당 페이지내에서만 공유)
		 *    
		 * 위 4개의 객체들에 
		 * 데이터를 담을 때 => .setAttribue("키",0);
