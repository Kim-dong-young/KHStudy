    ...
    <%-- 
    	*JSP를 이루는 구성
    	...
    	3. 액션태그 : jsp페이지에서 어떤 동작을 하도록 지시하는 태그
    				...
    				>> 표준 액션 태그(Standard Action Rag) : jsp페이지에서 바로 사용가능(별도의 연동 X )
    					-> 모든 태그명 앞에 jsp: 접두어를 붙여서 사용 <jsp: 태그명>
    				
    				>> 커스텀 액션 태그 : jsp페이지에서 바로 사용 불가능(별도의 라이브러리 연동)
    					-> 접두어에 jsp:가 있는 모든 태그를 제외한 모든태그
     ...
     <p>
     	...
     	공통적으로 자주 사용되는 코드들을 집합하여 보다 쉽게 사용할 수 있도록 태그화하여 표준으로 제공하는 라이브러리<br>
     </p>
     ...
     <p>
     	...
     	&lt;%@ taglib prefix="내가 앞으로 쓸 접두어" uri="라이브러리 파일상의 uri주소" %&gt;
     </p>
