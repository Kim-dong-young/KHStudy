...
public class Run {
	 ...
	 * (실제로 상속된 클래스를 통해서 객채 생성 시, 부모 클래스의 객체도 생성된다.
	 * 
	 * 상속의 장점
	 ...
	 *  
	 * 상속의 특징
	 *  - 클래스간의 다중상속이 불가능 ( 부모는 하나다 )
	 *  - 부모 클래스에 정의되어있는 protected 필드는 자식 클래스에서 직접 접근이 가능(private는 불가)
	 *  - 자식객체는 부모 클래스에 있는 메소드를 마치 내 것 처럼 호출할 수 있다.
	 *  	+ 부모 클래스의 정의된 기능이 마음에 들지 않는다면, 내 마음대로 수정할 수 있따.(오버라이딩)
