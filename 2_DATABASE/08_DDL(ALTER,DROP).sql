...
ALTER TABLE DEPT_TABLE ADD LNAME VARCHAR(20) DEFAULT '한국';
...
--ALTER TABLE DEPT_TABLE MODIFY DEPT_ID NUMBER; -- 이미 데이터가 있고 + 데이터가 형변환 불가능한 데이터라 오류

-- DEPT_TABLE에 있는 DEPT_TITLE을 VARCHAR2(40)으로 변경
...
-- ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID -- 실행불가, 테이블은 최소 하나이상의 컬럼이 필요
...
ALTER TABLE DEPT_TABLE DROP CONSTRAINT DTABLE_PK; -- 조건명 안썼을시, 임의로 부여된 제약조건명 찾아서 쓰면된다.
ALTER TABLE DEPT_TABLE 
...
-- 다른 테이블에 참조되고있는 부모테이블은 함부로 삭제가 되지 않는다.
-- 만약 지우고자 한다면
-- 1. 자식 테이블 먼저 전부 삭제
...
-- 3_2) 제약조건명 변겅 : RENAME CONSTRAINT 기존제약조건명 TO 바꿀제약조건명
ALTER TABLE DEPT_TABLE RENAME CONSTRAINT SYS_C007222 TO DTABLE_ID_NN;
...
----------------------------------------------------------------------------------------
-- TRUNCATE : 테이블 초기화 == DELETE 테이블명 이랑 같다.
