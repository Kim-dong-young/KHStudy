...
SELECT EMP_NAME, 
    SALARY * 12 AS "연봉",
    SALARY * ( 1 + NVL(BONUS,0) ) * 12 AS "총 수령액",
    TRUNC(SALARY * ( 1 + NVL(BONUS,0) ) * 12 - (SALARY * 12 * 0.03)) AS "실 수령액"
FROM EMPLOYEE;
    
-- 7. EMPLOYEE테이블에서 급여가 6000000~10000000인 사원의 이름, 월급, 고용일, 연락처 조회
...
SELECT EMP_NAME, 
    SALARY, 
    TRUNC(SALARY * 12 * ((1 + NVL(BONUS, 0)) - 0.03)) AS "실 수령액",
    HIRE_DATE
FROM EMPLOYEE
WHERE TRUNC(SALARY * 12 * ((1 + NVL(BONUS, 0)) - 0.03)) >= 50000000;
...
FROM EMPLOYEE;
-- 걍 SUBSTR(EMP_NO,1,2) , SUBSTR(EMP_NO,3,2) .. 해도되고
...
SELECT EMP_NAME,
    ABS(TRUNC(HIRE_DATE - SYSDATE)) AS 근무일수1,
...
-- 자동 형변환이 가능하다면 SQL이 자동으로 자동 형변환 해준다.
-- 이 경우 WHERE MOD(EMP_ID,2) = 1; 도 가능
...
SELECT EMP_NAME, TO_CHAR(SALARY,'L99,999,999') -- L : LOCAL
FROM EMPLOYEE;
...
SELECT EMP_NAME, 
    ...
    --TRUNC(MONTHS_BETWEEN(SYSDATE, SUBSTR(EMP_NO,1,6)) / 12) AS 나이
...
SELECT EMP_NAME, 
    ...
    -- TO_NUMBER(SUBSTR(EMP_NO,1,6)) + TO_NUMBER(SUBSTR(EMP_NO,8,14)) AS 합
    SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8) AS 합 -- 자동 형변환이 가능하다
FROM EMPLOYEE
WHERE EMP_ID = '201';
...
/*
    COUNT(*) - COUNT(NULLIF( EXTRACT(YEAR FROM HIRE_DATE),2001)) AS "2001년",
    COUNT(*) - COUNT(NULLIF( EXTRACT(YEAR FROM HIRE_DATE),2002)) AS "2002년",
    COUNT(*) - COUNT(NULLIF( EXTRACT(YEAR FROM HIRE_DATE),2003)) AS "2003년",
    COUNT(*) - COUNT(NULLIF( EXTRACT(YEAR FROM HIRE_DATE),2004)) AS "2004년"
*/
