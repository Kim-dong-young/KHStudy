...
/*
    ...
    [표현법]
    조건A AND 조건B -> 조건 A,B가 모두 만족하는 값만 참으로 간주한다
    조건A OR 조건B -> 조건 A와 조건B중 하나만 만족해도 참으로 간주한다
...
/*
    ...
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
        EX) 비교할 대상 컬럼 LIKE '문자%' -> 비교할 대상 컬럼값중 해당 문자로 시작하는 값들만 가져온다.
            비교할 대상 컬럼 LIKE '%문자' -> 비교할 대상 컬럼값중 해당 문자로 끝나는 값들만 가져온다.
            비교할 대상 컬럼 LIKE '%문자&' -> 비교할 대상 컬럼값중 해당 문자가 포함된 값 조회
            
    2. '_' : "딱 1글자"를 대체하는 검색
        EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할대상 컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
            비교할 대상 컬럼 LIKE '문자_' : 비교할대상 컬럼값 문자뒤에 아무글자나 한글자가 있는 값을 조회
            비교할 대상 컬럼 LIKE '_문자_' : 비교할대상 컬럼값 문자앞,뒤에 아무글자나 한글자가 있는 값을 조회
...
-- 사원들 중 이름에 '하'라는 글자가 포함된 사원의 사원명, 전화번호 조회

SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

-- 사원들 중 이름 중간에 '하'라는 글자가 포함된 사원의 사원명, 전화번호 조회
...
FROM EMPLOYEE
WHERE LENGTH( SUBSTR( EMAIL, 1, INSTR(EMAIL,'_') - 1 ) ) = 3;

SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
-- WHERE EMAIL LIKE '____%'; -> 와일드카드 문자떄문에 정상출력이 되지 않는다.
-- 와일드카드문자와 일반문자를 구분해줘야한다.
-- 데이터값으로 취급하고싶은 와일드카드 문자앞에 나만의 탈출문자를 재시해서 탈출시켜주면 된다.
-- ESCAPE OPTION을 등록해서 사용
WHERE EMAIL LIKE '___\_%' ESCAPE '\'; -- 마음대로 나만의 탈출문자를 등록하면 된다.
...
-- 4. 부서테이블에서 부서명이 해외영업부인 부서들의 부서코드, 부서명 조회
...
SELECT EMP_ID, EMP_NAME, SALARY, (SALARY * ( 1 + BONUS )) * 12 AS 연봉
FROM EMPLOYEE
