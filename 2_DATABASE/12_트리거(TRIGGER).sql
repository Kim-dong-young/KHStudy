/*
    ...
    예를들어 구매 테이블에 INSERT하면 (상품 구매하면) -> 자동으로 배송 테이블에 생성되어야 한다
    이런걸 미리 걸어둘 수 있다.
    ...
    입출고에 대한 데이터 기록(INSERT)될 때 마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야한다.
    ...
    * 트리거 생성 구문
    ...
    /         <-- 슬래쉬 까지 붙여줘야함. 그냥 문법임
*/
-- EMPLOYEE 테이블에 새로운 행이 INSERT될 때 마다 자동으로 출력되는 트리거 정의
SET SERVEROUTPUT ON; -- DBMS_OUTPUT.PUT_LINE ( SYSO 같은거 ) 실행을 위해 켜준다.
...
-- 상품번호 중복안되게 매번 새로운 번호를 발생시키는 시퀀스 생성
...
FOR EACH ROW -- 한 행이 INSERT 될 때마다 실행하겠다 , NEW OR OLD 사용에 필요
BEGIN -- PL/SQL 사용
    IF(:NEW.STATUS = '입고')
