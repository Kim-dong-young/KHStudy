 ...
 * 
 * IP주소 : 네트워크상에 각 컴퓨터들을 식별할 수 있는 번호(실제 주소와 동일한 역할을 한다고 생각하면 된다.)
 * 
 * * 서버와 클라이언트
 * * 서버 : 클라이언트(고객)에게 서비스를 제공해주는 프로그램 또는 컴퓨터 (요청에 대해서 응답해주는 것)
 * * 클라이언트 : 서버에 요청을 하는 컴퓨터(서비스를 제공받는 고객)
 * 
 * - 서버에 요청하기 위해서는 1. 요청하고자하는 서버의 IP주소(또는 도메인) 2. 포트번호를 알아야 함
 * * 현재 구동중인 서버가 있을 때
 ...
 * - UDP방식 : 데이터의 전송속도가 빠름! 신뢰성이 없는 데이터 전달에 용이(데이터를 빠른 속도로 전송하고자할 때)
 * 
 */
...
public class Run {
	public static void main(String[] args) {
		try {
			...
			System.out.println("google의 서버명 : "+googleHost.getHostName()); // 호스트의 이름을 반환
			System.out.println("google의 IP주소 : "+googleHost.getHostAddress()); // 호스트의 IP주소를 반환
