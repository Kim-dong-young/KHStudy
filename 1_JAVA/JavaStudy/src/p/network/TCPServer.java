...
public class TCPServer {
	 ...
	 * - 데이터를 교환하기에 앞서 서버, 클라이언트 연결이 되어야 한다.
	 *   (서버가 먼저 실행되에 클라이언트의 요청을 기다린다.)
	 * - 신뢰성있는 데이터 전달 가능
	 * 
	 * socket
	 * - 프로세스간의 통신을 담당
	 ...
	 * 
	 * ServerSocket
	 * - 포트와 연결되어 외부의 연결요청을 기다림 -> 요청이 들어오면 처리해준다.
	...
	public static void main(String[] args) {
		...
		// 1) 포트번호 지정(서버측에서 몇번 포트로 통로를 열 것인가?)
		int port = 3000;
		...
		try {
			...
			// 3) 요청을 기다렸다, 요청이 오면 -> 요청 수락 후, 클아이언트와 통신할 수 있는 서버쪽 소켓 객체 생성
			Socket socket = server.accept();
			System.out.println(socket.getInetAddress().getHostAddress() + "가 연결을 요청함...");
			
			// 4) 클라이언트와 입출력 기반 스트림 생성( 바이트스트림밖에 안됨 )
			// 5) 따라서 보조스트림을 활용해서 성능개선
			
			// 입력용 스트림(클라이언트로부터 전달된 값을 한줄 단위로 읽어들이기 위해서 입력용 스트림 사용)
			br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			// BufferedReader : 데이터 입력 시, nextLine메소드 이용 가능
